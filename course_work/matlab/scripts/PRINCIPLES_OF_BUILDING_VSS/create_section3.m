function create_section3(InitialConditions,PATH,section_name)
...
Cellname_subsection{2} =  'find_PD_neust_focus';
...
for i=1:length(Cellname_subsection)
    ...
    if strncmpi(name_subsection,'hole_',5) %нужно создать файл заглушку
        ...
        if strcmp(name_subsection,Cellname_subsection{1})
          [k{1}, p{1}] =find_PD(InitialConditions,PATH,section_name,name_subsection);    %находит коэффициенты для системы с фазовой траекторие типа "седло" .
        end
        if strcmp(name_subsection,Cellname_subsection{2})
          [ k{2}, p{2}]=find_PD_neust_focus(InitialConditions,PATH,section_name,name_subsection);    %находит коэффициенты для системы с фазовой траекторие типа "седло" .
        end
        if strcmp(name_subsection,Cellname_subsection{3})
          VSS_steady_degenerate_motion(InitialConditions,PATH,section_name,name_subsection,k,p);    
        end
        if strcmp(name_subsection,Cellname_subsection{4})
          VSS_no_steady_degenerate_motion(InitialConditions,PATH,path_subsection,name_subsection);    
        end
        if strcmp(name_subsection,Cellname_subsection{5})
          VSS_sliding_mode(InitialConditions,PATH,path_subsection,name_subsection,k,p);    
        end
    end
end
