function [FIGS,Settlingtime] = sim_VSS_final_DCS(PATH,InitialConditions,name_system,param,zad)
    ...
    % ѕеременные состо€ни€
    figure_name = [strrep(name_system,'sim_',''),'_sv_DCS_mal'];%им€ графика
    ...
    FIGS.description{1}=' √рафики изменени€ переменных состо€ни€ в <<малом>>.';
   
  
    % ѕеременные состо€ни€
    figure_name = [strrep(name_system,'sim_',''),'_sv_DCS_bol'];%им€ графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    FIGS.names{2} = figure_name ;
    FIGS.path{2}=figure_file_path;
    FIGS.description{2}=' √рафики изменени€ переменных состо€ни€ в <<большом>>.';
    
    
    num=3;
    figure_name = [strrep(name_system,'sim_',''),'_DCS_SAW'];%им€ графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    FIGS.names{num} = figure_name ;
    FIGS.path{num}=figure_file_path;
    FIGS.description{num}=' √енерируем пилу';
    ...
    for k=1:1
        ...
        %расчЄт максимума и минимума  y из всех значений
        if ymax<max(x)
        ymax=max(x);
        end
         if ymin>min(x)
    ...
    figure(Cellfig{1});%target figure
    %------------координаты линии времени регулировани€ линию
