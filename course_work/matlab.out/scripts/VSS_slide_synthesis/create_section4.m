  function create_section4(InitialConditions,PATH,section_name)
...
section_title = 'Синтез СПС со скользящим режимом методами фазового пространства' ;
%% Создание имен файлов и заглушек
Cellname_subsection{1} =  'VSS_3_order'; %имя файла для find_PD
...
for i=1:length(Cellname_subsection)
    ...
    if strncmpi(name_subsection,'hole_',5) %нужно создать файл заглушку
        ...
        if strcmp(name_subsection,Cellname_subsection{1})
           VSS_3_order(InitialConditions,PATH,path_subsection,name_subsection,section_name);    %находит коэффициенты для системы с фазовой траекторие типа "седло" .
        end
