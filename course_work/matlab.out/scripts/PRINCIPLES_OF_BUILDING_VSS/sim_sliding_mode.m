function FIGS = sim_sliding_mode(PATH,InitialConditions,k,p1,tau,name_system)
    ...
    % фазовые траектории
    figure_name = [strrep(name_system,'sim_',''),'_ft_SLM'];%имя графика
    ...
    figure_name = [strrep(name_system,'sim_',''),'_sv_SLM'];%имя графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    ...
    FStep=10^-2;
    %перенос переменных в основную базу переменных
    assignin('base','end_time',10);%------------------------------------------------------------------время симуляции1
...
for k=1:length(x0)
    ...
    if k==length(x0)
        ...
        text(ymax/1.5/p(1), ymax/1.5+step*1 ,txt1,'Interpreter','latex','FontSize',14); %подпись
        
        plot([ymin/tau ymax/tau],[ymin ymax],'k:','LineWidth',1);%строим 2 прямую сепаратрисы седловой траектории
        
        txt2 = ['$$y\,=\,',num2str(tau,3),'\,X$$'];   
        text(ymax/1/tau, ymax/1-step*4 ,txt2,'Interpreter','latex'); %подпись
