function FIGS = sim_sedlo(PATH,InitialConditions,k,p)
...
R=50;
y0=[1/sqrt(2)       ,   1/sqrt(2)  ,   1/sqrt(2)    ,     -1/sqrt(2)               ,    -1/sqrt(2)     ,   -1/sqrt(2)                ];
x0=[ -1.1/sqrt(2)      ,  -1/sqrt(2)  , -0.9/sqrt(2)   ,     1.1/sqrt(2)   ,       1/sqrt(2)    ,  0.9/sqrt(2)      ];
x0=x0*R;
y0=y0*R;
k1 = k(1);
k2 = k(2);
FStep=10^-2;
%перенос переменных в основную базу переменных
    assignin('base','end_time',3);%------------------------------------------------------------------время симуляции1
...
assignin('base','p',p);
    ...
    %leg2{2*k-1} =['$x_0=',num2str(x0(k),3),',y_0=',num2str(y0(k),3),'$'] ;
    if k==length(x0)
        step=(ymax-ymin)/21;
        plot([ymin/p(1) ymax/p(1)],[ymin ymax],'k:','LineWidth',1);%строим  1 прямую сепаратрисы седловой траектории
        
        txt1 = ['$$y\,=\,',num2str(p(1),3),'\,X$$'];   
        text(ymax/2.5/p(1), ymax/2.5-step*4 ,txt1,'Interpreter','latex','FontSize',14); %подпись
