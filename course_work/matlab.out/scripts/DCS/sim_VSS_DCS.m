...
%     num=1;
%     figure_name = [strrep(name_system,'sim_',''),'_DCS_SAWnew'];%имя графика
...
%     FIGS.description{num}=' Переворачиваем пилу  и поднимаем её над осью абсцис';
%     
    num=1;
    figure_name = [strrep(name_system,'sim_',''),'_DCS_pwm1'];%имя графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    FIGS.names{num} = figure_name ;
    FIGS.path{num}=figure_file_path;
    FIGS.description{num}=' ШИМ1';
    
   
     num=2;
    figure_name = [strrep(name_system,'sim_',''),'_DCS_pwm2'];%имя графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    FIGS.names{num} = figure_name ;
    FIGS.path{num}=figure_file_path;
    FIGS.description{num}=' ШИМ2';
    
    num=3;
    figure_name = [strrep(name_system,'sim_',''),'_DCS_PWM'];%имя графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    FIGS.names{num} = figure_name ;
    FIGS.path{num}=figure_file_path;
    FIGS.description{num}=' Воздействие на объект (ШИМ=ШИМ1+ШИМ2)';
    
    num=4;
    figure_name = [strrep(name_system,'sim_',''),'_DCS_upr'];%имя графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    FIGS.names{num} = figure_name ;
    FIGS.path{num}=figure_file_path;
    FIGS.description{num}=' управляющий непрерывный сигнал ';
    
    num=5;
    figure_name = [strrep(name_system,'sim_',''),'_DCS_upr_ogr'];%имя графика
    figure_file_path=['images/',figure_name];% путь к графику
    figure_file_path = join(figure_file_path);
    FIGS.names{num} = figure_name ;
    FIGS.path{num}=figure_file_path;
    FIGS.description{num}=' управляющий непрерывный сигнал после насыщения';
...
%          plot(t(start:ended),saw(start:ended),'k-','LineWidth',2);grid on,hold on ; 
    ...
    START_PR=10; %начало в процентах от длины всей симуляции
    LEN_PR=0.05; %длина в процентах от длины всей симуляции
    figure(Cellfig{2});
    count=end_time/FStep;
   ...
   % assignin('base','S_T2',ended);
    ...
    START_PR=0; %начало в процентах от длины всей симуляции
    LEN_PR=60; %длина в процентах от длины всей симуляции
        figure(Cellfig{5});
    count=end_time/FStep;
    start=int64((START_PR/100)*count);
    if start==0
